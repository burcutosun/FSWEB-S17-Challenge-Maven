...
public class CourseController {
    ...
    public Course getByName(@PathVariable String name){
        ...
        return courses.stream().filter(course-> course.getName().equalsIgnoreCase(name)).findFirst().orElseThrow(()->new ApiException("Course not found with given name:"+name, HttpStatus.NOT_FOUND));
    }
    ...
    private Course getExistingCourseById(Integer id) {
        return courses.stream().filter(course->course.getId().equals(id)).findFirst().orElseThrow(()->new ApiException("Course not found with id:"+id,HttpStatus.NOT_FOUND));
    }
